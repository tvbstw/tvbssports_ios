...
open class SKZoomingScrollView: UIScrollView {
    ...
    open func displayImage(complete flag: Bool) {
        ...
        if let image = photo.underlyingImage, photo != nil {
            // image
            //修改套件:使用Kingfisher支援gif格式
            //edit by Eddie 2022/05/06
            //imageView.image = image
            imageView.kf.setImage(with: URL(string:photo.photoURL), placeholder: UIImage(named:"default"), options: [.transition(ImageTransition.fade(IMAGE_FADE_SEC)), .processor(WebPProcessor.default), .cacheSerializer(WebPSerializer.default)], progressBlock: nil, completionHandler: nil)
