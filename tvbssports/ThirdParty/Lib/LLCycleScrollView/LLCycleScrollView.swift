...
open class LLCycleScrollView: UIView, UICollectionViewDelegate, UICollectionViewDataSource, UIScrollViewDelegate {
    ...
    //修改套件 重置index 20180712 Edtor by Eddie
    func reloadScrollToIndex(targetIndex: Int) {
        collectionView.scrollToItem(at: IndexPath.init(item: targetIndex, section: 0), at: position, animated: false)
    }
        ...
        if isOnlyTitle && titles.count > 0{
            ...
            if imagePaths.count == 0 {
                ...
                if imagePath.hasPrefix("http") {
                    ...
                    // 20180417 修改套件 支援Webp Eddie
                    cell.imageView.kf.setImage(with: URL(string: isAddingPercentEncodingForURLString ? escapedString ?? imagePath : imagePath), placeholder: placeHolderImage, options: [.transition(ImageTransition.fade(IMAGE_FADE_SEC)), .processor(WebPProcessor.default), .cacheSerializer(WebPSerializer.default)], progressBlock: nil, completionHandler: nil)
                }else{
                    /*
